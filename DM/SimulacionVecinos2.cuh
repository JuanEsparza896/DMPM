#ifndef SIMVEC2_HEADER
#define SIMVEC2_HEADER
...
__global__ void CalculoDeVecinos2(uint np,uint n_esp_p,uint *M_int,uint *esp_de_p,int chp,const double *p, int *vecinos, unsigned int *n_vecinos, int nmaxvec,int3 condper,uint3 *mad_de_p, double3 caja,double3 cajai,double rc)
{
    ...
    unsigned int my_n_max = nmaxvec;//esto es nmaxvec
    ...
    while (!done)
    {
        ...
        if (lane == 0)
        n_vecinos[particula]= nneigh;
    }
}

__global__ void AceleracionesfFuerzasLJV2(uint np,uint n_esp_p,uint pot_int,uint *esp_de_p,const double *p,int chp,double *M_param,double3 caja,double3 cajai,int3 condper,double *epot,double *a,int *vec,unsigned int *nvec,int nmaxvec,double rc,bool nconf)
{
                 ...
                 size_t memoria_global,std::ofstream &ofasres,std::ofstream &ofasat)
{
    ...
    CalculoDeVecinos2<<<blockspergridvec,threadsperblockvec>>>(np,n_esp_p,d_M_int,d_esp_de_p,chp,d_pos,d_vec,d_nvec,nmaxvec,condper,d_mad_de_p,caja,cajai,rcc);
