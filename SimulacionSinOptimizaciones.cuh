    ...
    for(int j=lane;j<np;j+=chp){
        ...
        if(M_int[n_esp_p*esp1+esp2]){
            if(m_de_p[gid].x-m_de_p[j].x){
                ...
                dis=Discuad(dif);
                fuepot=Interaccion(pot,n_esp_p,elem_M,gid,j,dis,s_mem,nconf,false);
...
void Simulacion(uint nc,uint ncp,uint nhilos,uint np,uint nparam,uint pot,uint n_esp_p,uint3 *mad_de_p,uint *esp_de_p,uint *M_int,int maxhilos,
            size_t memoria_global,double dt,double dens,double *param,double *pos,double *vel,double *acel,int3 condper,double3 caja, double3 cajai,std::ofstream &ofasres,std::ofstream &ofasat)
{
    ...
    double s_nh_a = 0.,s_nh_v=0.,s_nh_p=0.;
