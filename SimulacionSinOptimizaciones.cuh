...
__global__ void AceleracionesFuerzas(uint n_esp_m,uint n_esp_p,uint nparam,uint pot,uint np,uint *m_de_p,uint *esp_de_p,uint *M_int,const double *pos,int chp,double *M_param,double3 caja,double3 cajai,int3 condper,double *epot,double *a,bool nconf)
{
    ...
    for(int j=lane;j<np;j+=chp){
            if(!(n_esp_p-1)){
                esp2=esp_de_p[j];
                if(!M_int[n_esp_p*gid+j])break;
            }
            if(!(m_de_p[gid]-m_de_p[j]))break;
            pjx=__ldg(pos+3*j);
            pjy=__ldg(pos+3*j+1);
            pjz=__ldg(pos+3*j+2);
            ...
            dis=Discuad(dif);
            fuepot=Interaccion(pot,n_esp_p,gid,j,dis,M_param,nconf,false);
...
void Simulacion(uint nc,uint ncp,uint nhilos,uint np,uint nparam,uint pot,uint n_esp_p,uint n_esp_m,uint *m_de_p,uint *esp_de_p,uint *M_int,int maxhilos,
            size_t memoria_global,double dt,double dens,double *param,double *pos,double *vel,double *acel,int3 condper,double3 caja, double3 cajai,std::ofstream &ofasres,std::ofstream &ofasat)
{
    ...
    uint *d_m_de_p,*d_esp_de_p,*d_M_int;
    double *d_eit,*d_pos,*d_acel,*d_M_param;

    Errorcuda(cudaMalloc(&d_m_de_p,np*sizeof(uint)),"m_de_p",0);
    Errorcuda(cudaMalloc(&d_esp_de_p,np*sizeof(uint)),"esp_de_p",0);
    Errorcuda(cudaMalloc(&d_M_int,n_esp_p*n_esp_p*sizeof(uint)),"m_de_p",0);
    ...
    Errorcuda(cudaMemcpy(d_m_de_p,m_de_p,np*sizeof(uint),cudaMemcpyHostToDevice),"m_de_p",1);
    ...
    AceleracionesFuerzas<<<blockspergrid,threadsperblock>>>(n_esp_m,n_esp_p,nparam,pot,np,d_m_de_p,d_esp_de_p,d_M_int,d_pos,chp,d_M_param,caja,cajai,condper,d_eit,d_acel,nconf);
    ...
    for(ic=0;ic<nc;ic++){
        ...
        for(ip=0; ip<np; ip++){
            for(id=0; id<nd; id++)pos[id+ip*nd] +=(vel[id+ip*nd] * dt + acel[id+nd*ip]*dt*dt*0.5);
